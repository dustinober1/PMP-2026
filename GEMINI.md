...
## Recent Changes
- **2026-01-07**: Fixed rendering issue in `guide/09-monitoring/monitoring-closing.md` where the "Decision Tree: Where Do I Log This?" ConceptCard was displaying raw HTML as a code block. The issue was caused by blank lines within the HTML content inside the Vue component, which caused VitePress's markdown parser to exit HTML mode. Removed blank lines to ensure proper rendering.
- **2026-01-07**: Modernized `guide/10-ai-pm/toolkit.md` by replacing all code blocks, ASCII art, and markdown tables with styled `ConceptCard` components. Converted the AI Working Agreement Template, Data Redaction Checklist, Prompt Template, HITL Review Checklist, AI Risk Register Add-On (8 common risks), Vendor Due Diligence Checklist, Pilot-to-Production 7-Step Roadmap, AI Risk Register Template, Quality Monitoring Checklist, AI Decision Log, Exam Quick Picks, Expanded Prompt Library (11 prompts across Initiation, Planning, Execution, M&C, Closing), ROI Calculator with formula and decision table, Tool Comparison Matrix, Escalation Decision Tree, and AI Maturity Assessment (4 levels with interpretation guide) into visually appealing interactive cards with gradient backgrounds, color-coded badges, and icons. Verified successful build.
- **2026-01-07**: Deleted practice exam pages (`practice-exam-1.md`, `practice-exam-2.md`, `practice-exam-3.md`) from `guide/11-exam-prep/` and removed their sidebar links from `.vitepress/sidebar/chapters.mjs` since practice exams are not ready for launch. Verified successful build.
...
- **2026-01-14**: Audited all 5 practice exam JSON files for distractor length compliance. Created \`scripts/audit_json_exam.py\` to check if distractors are within ±10% of correct answer length. Used an LLM-powered fix script to rewrite non-compliant distractors. Final compliance rates at ±15% tolerance: Exam 1 (96.8%), Exam 2 (96.8%), Exam 3 (99.5%), Exam 4 (99.5%), Exam 5 (97.3%). Renamed fixed files to standard names (\`practice-exam-1.json\` through \`practice-exam-5.json\`).
- **2026-01-14**: Enhanced \`ExamSimulator.vue\` with comprehensive performance analysis features: added domain-wise breakdown with visual progress bars, color-coded score display (excellent/pass/borderline/needs-work), strong/weak areas identification by domain-task combination, and study recommendations based on weak areas. Questions and answers continue to be randomized each attempt using deterministic seeding.
- **2026-01-14**: Added "Buy Me a Coffee" donation button to support the free resource. Created `BuyMeCoffee.vue` component with tasteful styling. Added a subtle "Enjoying This Free Resource?" support section at the bottom of the homepage with a friendly, non-pushy message. The button links to https://www.buymeacoffee.com/dustinober.
- **2026-01-14**: Created reusable `AudioPlayer.vue` component for chapter narration. Established naming convention `/audio/ch{XX}-introduction.mp3`. Converted Chapter 1 WAV (5.9MB) to MP3 (1.6MB - 73% size reduction). Component accepts `:chapter` prop for easy integration. Usage: `<AudioPlayer :chapter="1" />`. Registered as global component in theme.
- **2026-01-14**: Extended donation button to appear in the footer of every documentation page. Created `CustomLayout.vue` that extends the default VitePress theme and adds a subtle "Enjoying this free study guide?" prompt with the Buy Me a Coffee button in the `doc-footer-before` slot.
