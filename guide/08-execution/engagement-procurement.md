...
An alternative model focusing on **questions** the team must answer:

1. **Orientation**: Why am I here? (Purpose clarity)
2. **Trust Building**: Who are you? (Relationships)
3. **Goal Clarification**: What are we doing? (Shared vision)
4. **Commitment**: How will we do it? (Approach agreement)
5. **Implementation**: Who does what, when, where? (Execution)
6. **High Performance**: Wow! (Synergy and flow)
7. **Renewal**: Why continue? (Sustaining momentum)

### Team-Building Activities

- **Co-location**: Physical proximity improves communication (war rooms, team spaces).
- **Training**: Skill development that also builds camaraderie.
- **Team-Building Events**: Offsite workshops, social activities, trust exercises.
...
People are motivated by unfulfilled needs, progressing from basic to higher-level needs:
...
```
       ...
       (Recognition, achievement, respect)
              ↑
          Social
      (Belonging, teamwork, relationships)
              ↑
          Safety
     (Job security, safe working conditions)
              ↑
       Physiological
    (Salary, breaks, comfortable workspace)
```
...
- **Expectancy**: "If I work hard, will I succeed?" (Effort → Performance)
- **Instrumentality**: "If I succeed, will I get a reward?" (Performance → Outcome)
...
**Outcome**: Buys time, but conflict remains unresolved. Can escalate if avoided too long.
...
### The Procurement Execution Flow

```
Plan Procurement → Request Seller Responses → Evaluate Proposals →
Select Seller → Negotiate Contract → Award Contract → Control Procurements
```
...
**Weighting System**:
Assigns points based on how well each proposal meets criteria.

**Example**:
| Vendor | Technical (40%) | Price (30%) | Past Performance (15%) | Management (10%) | Financial (5%) | **Total** |
|---|---|---|---|---|---|---|
| Vendor A | 36/40 | 25/30 | 12/15 | 9/10 | 5/5 | **87/100** |
| Vendor B | 38/40 | 20/30 | 15/15 | 8/10 | 4/5 | **85/100** |
| Vendor C | 32/40 | 30/30 | 10/15 | 7/10 | 5/5 | **84/100** |

**Result**: Vendor A wins (highest total score), even though Vendor C had the lowest price. The weighting system allowed technical capability to outweigh cost.
...
Negotiation is a **collaborative process** to reach a fair agreement. The goal is **win-win**, not to exploit the vendor.

**Key Principle**: A bad deal for the seller often leads to poor performance, shortcuts, or even contract abandonment. Fair pricing ensures vendor commitment.
...
- **You may not lead negotiations**: Procurement specialists or legal teams often lead.
- **You provide technical expertise**: Clarify scope, validate feasibility, assess risk.
...
- **Change Control**: Process for modifying the contract.
...
- **Maintaining documentation**: Keeping records of correspondence, decisions, changes, performance data.
- **Managing risks**: Identifying and mitigating procurement-related risks.
...
4. **Contract Amendment**: Formalize the change in a contract amendment (also called a "change order" or "contract modification").
...
### Claims Administration & Dispute Resolution
...
1. **Claim Notification**: The claimant (seller or buyer) formally notifies the other party, citing the contract clause and the basis for the claim.
...
- **Resolution**: Buyer reviews records, confirms the delay. Buyer's PM admits fault. Parties negotiate a settlement: Buyer agrees to $10,000 additional payment (compromise). Contract is amended.
...
4. **Archive Documentation**: Store all procurement records (contract, amendments, correspondence, inspection reports, payment records) per organizational policy.
5. **Lessons Learned**: Document what went well and what didn't (vendor performance, contract effectiveness, process improvements).
...
- **Final Invoice**: Received June 20. Approved and paid July 1.
- **Procurement Closed**: July 1 (acceptance + payment complete).
...
- **Negotiation**: You negotiate down to $45,000 and 10 days.
...
- **Claims & Disputes**: Always start with the contract. Use negotiation, then ADR (mediation/arbitration), litigation is the last resort.
- **Formal Closure**: Final acceptance + final payment + lessons learned.
...
<FlashcardCarousel :cards="[
  { front: `What are the 5 Tuckman stages?`, back: `Forming, Storming, Norming, Performing, Adjourning.` },
  { front: `Herzberg Hygiene Factors vs. Motivators?`, back: `Hygiene = prevent dissatisfaction (salary, conditions). Motivators = create satisfaction (achievement, recognition).` },
  { front: `What is the best conflict resolution approach per PMI?`, back: `Collaborating/Problem-Solving - work together for a win-win solution that satisfies all parties.` },
  { front: `What is the PTA formula?`, back: `PTA = ((Ceiling Price - Target Price) / Buyer Share Ratio) + Target Cost.` },
  { front: `FFP vs. CPFF contracts - who bears risk?`, back: `FFP = Seller bears cost risk. CPFF = Buyer bears cost risk.` },
  { front: `What is a Bidder Conference?`, back: `A meeting with all prospective sellers to clarify SOW and ensure all vendors have the same information.` },
  { front: `Theory X vs. Theory Y?`, back: `Theory X = people dislike work, need control. Theory Y = people are self-directed and seek responsibility.` },
  { front: `What is a Constructive Change?`, back: `When buyer actions effectively change scope without a formal change order - seller may claim compensation.` },
  { front: `What are the 5 TKI conflict modes?`, back: `Competing, Collaborating, Compromising, Avoiding, Accommodating.` },
  { front: `What is the first step in dispute resolution?`, back: `Check the contract first - it is the authoritative document for all procurement disputes.` }
]" />
